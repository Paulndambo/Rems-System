...
from .serializers import (
    UnitListingSerializer,
    ListingImageSerializer,
    ClientRequestSerializer,
    AmenitySerializer,
    CommentSerializer,
    ListingInterestExpressionSerializer,
    CollectListingViewsSerializer,
    UploadListingImageSerializer,
)
...
class UnitListingListView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if town:
            town = town.strip()
            filter_conditions &= Q(city__name__icontains=town) | Q(
                city__county__icontains=town
            )
        ...
        if min_price:
            try:
                ...
                return Response(
                    {"error": "Invalid min_price value. It must be a number."},
        ...
        if max_price:
            try:
                ...
                return Response(
                    {"error": "Invalid max_price value. It must be a number."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            for amenity in amenities_list:
                listing_amenities.append(
                    UnitAmenity(unit_listing=listing, name=amenity)
                )
            ...
            ListingImage.objects.create(
                listing=listing,
                image=data["unit_image"],
                image_url="http://localhost.com",
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid(raise_exception=True):
            try:
                ...
                file_content = image_file.read()
                file_content = ContentFile(file_content)
                file_name = fs.save(f"temp_source_file.{file_extension}", file_content)
                temp_file = fs.path(file_name)

                cloudinary_handler = CloudinaryHandler()
                upload_result = cloudinary_handler.upload_image(
                    temp_file, "listing_images"
                )
                ...
                return Response(
                    {"data": serializer.data, "upload_result": upload_result},
...
class UploadListingImageAPIView(APIView):
    ...
    def post(self, request, listing_id, *args, **kwargs):
        ...
        for image in images:
            ...
            upload_result = cloudinary_handler.upload_image(temp_file, "listing_images")
            ListingImage.objects.create(
                listing=listing, image=image, image_url=upload_result["public_url"]
            )
            cloudinary_handler.clean_up_temp_file(temp_file)
        return Response(
...
class CollectListingViewsAPIView(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid(raise_exception=True):
            listing = UnitListing.objects.filter(
                id=serializer.validated_data.get("listing_id")
            ).first()
            if listing:
                listing.views += 1
                listing.save()
                return Response(
                    {"message": "Listing views increased by 1"},
                    status=status.HTTP_201_CREATED,
                )
            return Response(
                {"failed": "Listing views could not be increased by 1"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
