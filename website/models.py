...
class Lister(AbstractBaseModel):
    owned_by = models.ForeignKey(
        "users.User", on_delete=models.SET_NULL, null=True, related_name="listersowners"
    )
...
class UnitListing(AbstractBaseModel):
    lister = models.ForeignKey(
        Lister, on_delete=models.CASCADE, related_name="listings", null=True, blank=True
    )
    ...
    unit_price_currency = models.CharField(
        max_length=255, choices=CURRENCY_CHOICES, default="KES"
    )
    ...
    minimum_lease_period = models.CharField(
        max_length=255, choices=MINIMUM_LEASE_PERIOD_CHOICES, default="1 Month"
    )
    notice_period = models.CharField(
        max_length=255, choices=NOTICE_PERIOD_CHOICES, default="1 Month"
    )
    furnish_status = models.CharField(
        max_length=255, choices=FURNISH_STATUS_CHOICES, default="Unfurnished"
    )
    ...
    direct_contact_fee = models.DecimalField(
...
class UnitAmenity(AbstractBaseModel):
    unit_listing = models.ForeignKey(
        UnitListing, on_delete=models.CASCADE, related_name="amenities"
    )
    name = models.CharField(max_length=255)
...
class ListingImage(AbstractBaseModel):
    listing = models.ForeignKey(
        UnitListing, on_delete=models.CASCADE, related_name="images"
    )
...
class ListingInterestExpression(AbstractBaseModel):
    listing = models.ForeignKey(
        UnitListing, on_delete=models.CASCADE, related_name="listinginterests"
    )
