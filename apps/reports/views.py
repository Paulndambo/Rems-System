...
def monthly_rent_report(request):
    ...
    selected_year = (
        selected_year
        if selected_year
        else Year.objects.get(name=str(date_today.year)).id
    )
    # Base queryset
    ...
    if selected_year:
        ...
        for month in MONTHS_LIST:
            ...
            month_expected = (
                month_bills.aggregate(total_expected=Sum("amount_expected"))[
                    "total_expected"
                ]
                or 0
            )
            month_paid = (
                month_bills.aggregate(total_paid=Sum("amount_paid"))["total_paid"] or 0
            )
    ...
    if export == "csv":
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="rent_bills_{selected_month or "all"}_{year.name if year else "all" or "all"}.csv"'
        )
        ...
        total_amount_expected = (
            rent_bills.aggregate(total_expected=Sum("amount_expected"))[
                "total_expected"
            ]
            or 0
        )
        total_amount_paid = (
            rent_bills.aggregate(total_paid=Sum("amount_paid"))["total_paid"] or 0
        )
        ...
        writer.writerow(["", "", "", "", "", "", "", ""])
        writer.writerow(
            ["", "", "", "", "Total Expected", total_amount_expected, "", ""]
        )
...
def water_payments_report(request):
    ...
    if month and tenant:
        water_payments = water_payments.filter(
            water_bill__year_id=year,
            water_bill__month__name=month,
            water_bill__tenant_id=tenant,
        )
        chart_data_query = chart_data_query.filter(
            year_id=year, month__name=month, tenant_id=tenant
        )

    elif month:
        water_payments = water_payments.filter(
            water_bill__year_id=year, water_bill__month__name=month
        )
        ...
        water_payments = water_payments.filter(
            water_bill__year_id=year, water_bill__tenant_id=tenant
        )
    ...
    chart_data = (
        chart_data_query.values("month__name", "year__name")
        .annotate(
