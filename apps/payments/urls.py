...
urlpatterns = [
    ...
    path(
        "monthly-rent-bills/", MonthlyRentBillsView.as_view(), name="monthly-rent-bills"
    ),
    path(
        "rent-receipts/<int:month_id>/",
        RentReceiptsView.as_view(),
        name="rent-receipts",
    ),
    ...
    path(
        "rent-payments-overview/", rent_payments_overview, name="rent-payments-overview"
    ),
    path(
        "caretaker-rent-bills/",
        CaretakerRentBillsView.as_view(),
        name="caretaker-rent-bills",
    ),
    # Garbage Bills
    path("garbage-bills/", GarbageBillsView.as_view(), name="garbage-bills"),
    path(
        "generate-garbage-bills/", generate_garbage_bills, name="generate-garbage-bills"
    ),
    ...
    # Unit Bills
    path(
        "monthly-unit-bills/", MonthlyUnitBillsView.as_view(), name="monthly-unit-bills"
    ),
    ...
    path(
        "collect-unit-bill-payment/",
        collect_unit_bill_payment,
        name="collect-unit-bill-payment",
    ),
    path(
        "unit-bill-receipt/<int:unit_bill_id>/",
        unit_bill_receipt,
        name="unit-bill-receipt",
    ),
    path("pending-bills/", PendingBillsView.as_view(), name="pending-bills"),
    path("generate-bill/", generate_bill, name="generate-bill"),
    # Security Deposits
    path(
        "security-deposits/", SecurityDepositsView.as_view(), name="security-deposits"
    ),
