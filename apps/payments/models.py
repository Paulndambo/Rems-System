...
class WaterBill(AbstractBaseModel):
    unit = models.ForeignKey("properties.PropertyUnit", on_delete=models.CASCADE, related_name="unitwaterbills")
    ...
    status = models.CharField(max_length=255, default=PaymentStatuses.PENDING.value, choices=PaymentStatuses.choices())
...
class TenantPayment(AbstractBaseModel):
    ...
    def save(self, *args, **kwargs):
        self.amount_expected = self.rent_bill.amount + self.water_bill.amount + self.previous_balances
        super().save(*args, **kwargs)
