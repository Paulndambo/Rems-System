...
class Property(AbstractBaseModel):
    ...
    def occupancy_rate(self):
        occupancy = (
            (self.occupied_units() / self.total_units()) * 100
    ...
    def monthly_revenue(self):
        return sum(
            list(
                self.propertyunits.filter(is_occupied=True).values_list(
                    "rent", flat=True
                )
            )
        )
...
class WaterBill(AbstractBaseModel):
    ...
    def total_amount(self):
        ...
        total_cost = (Decimal(water_price) * Decimal(self.units_consumed)) + Decimal(
    ...
    def save(self, *args, **kwargs):
        ...
        if self.month and self.year:
            ...
            first_day_next_month = date(year_number, month_number, 1) + timedelta(
                days=calendar.monthrange(year_number, month_number)[1]
            )
    ...
    def refresh_bill(self):
        self.units_consumed = Decimal(self.current_reading) - Decimal(
...
class ElectricityBill(AbstractBaseModel):
    ...
    unit = models.ForeignKey(
        PropertyUnit, on_delete=models.CASCADE, related_name="unitelectricitybills"
    )
